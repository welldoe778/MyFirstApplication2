package com.mocoo.hang.rtprinter.pdf;
...
import com.mocoo.hang.rtprinter.main.MainActivity;
...
/*				if(RTApplication.uri!=null){
					uri = RTApplication.uri;
					RTApplication.uri=null;
					RTApplication.uristatus = 0;*/
				//}else{
					uri = intent.getData();
				//}
	...
	//Bitmap 20160717
public Bitmap[] createBitmaps(int startPage,int endPage){
	  Bitmap[] bms=new Bitmap[(endPage - startPage)+1];
	  for(int i=startPage;i<endPage+1;i++){
		  bms[i-startPage]=core.drawPage(i,(int)core.getPageSize(i).x,(int)core.getPageSize(i).y,0,0, (int)core.getPageSize(i).x,(int)core.getPageSize(i).y);
	  } 
	  return bms;
	}
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case OUTLINE_REQUEST:
			...
			break;
		case PRINT_REQUEST:
			Toast.makeText(this, "TEST", Toast.LENGTH_LONG).show();//4000
			break;
		}
	...
	public void OnPrintButtonClick(View v) {
		...
		if (RTApplication.getConnState() == Contants.UNCONNECTED) {
            ToastUtil.show(MuPDFActivity.this, R.string.tip_connect);


           // RTApplication.uristatus = 1;
            try {
				Thread.sleep(100);
...
//		if(RTApplication.mode==Contants.CONNECTED_BY_USB){
//			 ToastUtil.show(MuPDFActivity.this, R.string.UsbPdf);
//			 return;
//		}
		popup();
	}
...
private void popup(){
		...
		View contentView = inflater.inflate(R.layout.pdfprint_copies,null);

		final PopupWindow popupWindow = new PopupWindow(contentView,LayoutParams.FILL_PARENT,LayoutParams.FILL_PARENT,true);
		View paretnView = inflater.inflate(R.layout.activity_main, null);
		popupWindow.setAnimationStyle(R.style.mypopwindow_anim_style);
		popupWindow.showAtLocation(paretnView, Gravity.CENTER,0,0);
		final EditText copies=(EditText)contentView.findViewById(R.id.printcopies);
		TextView quxiaoTv= (TextView)contentView.findViewById(R.id.print_copies_quxiao);
		TextView confirmTv= (TextView)contentView.findViewById(R.id.print_copies_confirm);
		final EditText from = (EditText)contentView.findViewById(R.id.pdf_print_from);
		final EditText to = (EditText)contentView.findViewById(R.id.pdf_print_to);
		final int pagecount = core.countPages();
		to.setText(""+pagecount);
		
		quxiaoTv.setOnClickListener(new OnClickListener() {
			@Override
		...
		confirmTv.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View view) {
				String labelCopies=copies.getText().toString();				
				RTApplication.labelCopies=labelCopies;
				fromPage = from.getText().toString();
				toPage = to.getText().toString();
				popupWindow.dismiss();
				 try {
						print();
					 //Bitmap[] bitmap = createBitmaps(Integer.parseInt(fromPage)-1,Integer.parseInt(toPage)-1);
					 //print(bitmap);
				} catch (Exception e) {
					// TODO 自动生成的 catch 块
					e.printStackTrace();
...
//	String twices=RTApplication.betwices;
// 	String time=RTApplication.betime;
	LogUtils.d("mupdf","mode="+RTApplication.mode);
	LogUtils.d("mupdf","getConnState="+RTApplication.getConnState());

    switch (RTApplication.getConnState()) {
        case Contants.CONNECTED_BY_BLUETOOTH:
            HsBluetoothPrintDriver hsBluetoothPrintDriver = HsBluetoothPrintDriver.getInstance();
		//	ToastUtil.show(context,"开始打印");
         	   hsBluetoothPrintDriver.Begin();   
//         	   if(RTApplication.beeper.equals("-1")){
//           		   hsBluetoothPrintDriver.Beep((byte)Integer.parseInt(twices), (byte)Integer.parseInt(time));
//             	   }
...
//          	   }
	          ...
	          Bitmap bmOne[];
		     for(int k = 0;k<Integer.parseInt(RTApplication.labelCopies);k++){        
		    	 for(int i=Integer.parseInt(fromPage)-1;i<Integer.parseInt(toPage);i++){
					 bmOne =  createBitmaps(i,i);
		            if(hsBluetoothPrintDriver.printImage(bmOne[0],0)){
...
//		                 switch( RTApplication.qiedaosetting){
//		                 case 0:
//		                	 break;
//		                 case 1:
...
//            if(RTApplication.beeper.equals("-1")){
//             	hsUsbPrintDriver.Beep((byte)Integer.parseInt(twices), (byte)Integer.parseInt(time));
//          	   }
//            if(RTApplication.cashbox==1){
...
//         	   }
            ...
            if(hsUsbPrintDriver.printImage(bmOne[0],0)){
                hsUsbPrintDriver.LF();
                hsUsbPrintDriver.CR();
                hsUsbPrintDriver.LF();
                hsUsbPrintDriver.CR();
...
//            if(RTApplication.beeper.equals("-1")){
//             	hsWifiPrintDriver.Beep((byte)Integer.parseInt(twices), (byte)Integer.parseInt(time));
//          	   }
//            if(RTApplication.cashbox==1){
...
//         	   }
            ...
            for(int k = 0;k<Integer.parseInt(RTApplication.labelCopies);k++){        
            	 for(int i=Integer.parseInt(fromPage)-1;i<Integer.parseInt(toPage);i++){
					 bmOne =  createBitmaps(i,i);
            if(hsWifiPrintDriver.printImage(bmOne[0],Contants.TYPE_80)){
...
private void labelPrint(Bitmap bm) {
    ...
    switch (RTApplication.mode) {
        case Contants.CONNECTED_BY_BLUETOOTH:
        	...
        	HsBluetoothPrintDriver labelBluetoothPrintDriver = HsBluetoothPrintDriver.getInstance();
