...
public class MuPDFPageView extends PageView implements MuPDFView {
	...
	private AsyncTask<Void,Void,MuPDFCore.PassClickResult> mPassClick;
	...
	public Hit passClickEvent(float x, float y) {
		...
		if (hit) {
			mPassClick = new AsyncTask<Void,Void,MuPDFCore.PassClickResult>() {
				@Override
				protected MuPDFCore.PassClickResult doInBackground(Void... arg0) {
					return mCore.passClickEvent(mPageNumber, docRelX, docRelY);
				}

				@Override
				protected void onPostExecute(MuPDFCore.PassClickResult result) {
					if (result.changed) {
						changeReporter.run();
					}
					...
					result.acceptVisitor(new PassClickResultVisitor() {
						@Override
						public void visitText(MuPDFCore.PassClickResultText result) {
							invokeTextDialog(result.text);
						}

						@Override
						public void visitChoice(MuPDFCore.PassClickResultChoice result) {
